...
public class IpNetwork extends Network implements Runnable {
	...
	public void initialize(Transport transport) throws Exception {
		...
		// broadcastAddress = new Address(broadcastIp, port, new Network(0xffff,
		// new byte[0]));
	...
	/**
	 * Sends a foreign device registration request to addr. On successful
	 * registration (AKN), we are added in the foreign device table FDT.
	 * 
	 * @param addr
	 *            The address of the device where our device wants to be
	 *            registered as foreign device
	 * @param timeToLive
