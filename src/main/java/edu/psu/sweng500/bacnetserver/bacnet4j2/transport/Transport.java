...
import edu.psu.sweng500.bacnetserver.bacnet4j2.apdu.Error;
import edu.psu.sweng500.bacnetserver.bacnet4j2.apdu.Reject;
import edu.psu.sweng500.bacnetserver.bacnet4j2.apdu.SegmentACK;
import edu.psu.sweng500.bacnetserver.bacnet4j2.apdu.Segmentable;
import edu.psu.sweng500.bacnetserver.bacnet4j2.apdu.SimpleACK;
import edu.psu.sweng500.bacnetserver.bacnet4j2.apdu.UnconfirmedRequest;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.exception.AbortAPDUException;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.exception.BACnetRejectException;
import edu.psu.sweng500.bacnetserver.bacnet4j2.exception.BACnetTimeoutException;
import edu.psu.sweng500.bacnetserver.bacnet4j2.exception.ErrorAPDUException;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.exception.RejectAPDUException;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.npdu.ip.SegmentWindow;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.type.error.BaseError;
...
public class Transport {
	...
	//
	/**
	 * Sends a confirmed service, and blocks until a reply is received, or
	 * timeout.
	 * 
	 * @param address
	 * @param linkService
	...
	public void sendUnconfirmed(Address address, OctetString linkService, UnconfirmedRequestService serviceRequest,
		...
		// Unconfirmed services will never have to be segmented, so just send
		// it.
	...
	//
	//
	// Receiving messages - messages are initiated externally, and received from
	// the data link. Just call the handler
	...
	private AckAPDU sendSegmented(WaitingRoomKey key, int maxAPDULengthAccepted, int maxServiceData,
		...
		// System.out.println("Send segmented: "+ (serviceData.size() /
		// maxServiceData + 1) +" segments required");
	...
	/**
	 * Waits for an acknowledgement. If the ack is complex and segmented, all of
	 * the segments will be read.
	 * 
	 * @param key
	 * @param timeout
	 * @param throwTimeout
	...
	private AckAPDU waitForAck(WaitingRoomKey key, long timeout, boolean throwTimeout) throws BACnetException {
		...
		try {
			...
			// Detect runtime exceptions in order to provide the original
			// service data that, we presume,
			// caused the problem.
	...
	void receiveSegmented(WaitingRoomKey key, Segmentable firstPart) throws BACnetException {
		...
		network.sendAPDU(key.getAddress(), key.getLinkService(),
				new SegmentACK(false, server, id, lastSeq, window, true), false);
		...
		while (true) {
			...
			if (segment == null) {
				...
				if (segment == null) {
					...
					// and start over.
					network.sendAPDU(key.getAddress(), key.getLinkService(),
							new SegmentACK(true, server, id, lastSeq, window, true), false);
			...
			// System.out.println("Received segment "+
			...
			if (!segment.isMoreFollows() || segmentWindow.isLastSegment(lastSeq)) {
				...
				network.sendAPDU(key.getAddress(), key.getLinkService(),
						new SegmentACK(false, server, id, lastSeq, window, segment.isMoreFollows()), false);
	...
	public void incomingApdu(APDU apdu, Address address, OctetString linkService) throws BACnetException {
		...
		// throw new MessageValidationAssertionException("Inconsistent message:
		// APDU expectsReply="+
		...
		if (apdu instanceof ConfirmedRequest) {
			...
			if (confAPDU.isSegmentedMessage() && confAPDU.getSequenceNumber() > 0)
				// This is a subsequent part of a segmented message. Notify the
				// waiting room.
				waitingRoom.notifyMember(address, linkService, invokeId, false, confAPDU);
			else {
				if (confAPDU.isSegmentedMessage()) {
					// This is the initial part of a segmented message. Go and
					// receive the subsequent parts.
				...
				try {
					...
					Error error = new Error(confAPDU.getInvokeId(), new BaseError((byte) 127,
			...
			// Used for testing only. This is required to test the parsing of
			// service data in an ack.
