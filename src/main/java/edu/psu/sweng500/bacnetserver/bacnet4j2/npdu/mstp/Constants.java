...
public class Constants {
	 ...
	 * allowable address for master nodes. The value of Max_Master shall be less
	 * than or equal to 127. If Max_Master is not writable in a node, its value
	 * shall be 127.
	 */
	public static final int MAX_MASTER = 127;

	/**
	 * The number of tokens received or used before a Poll For Master cycle is
	 * executed: 50.
	 */
	...
	/**
	 * The maximum time after the end of the stop bit of the final octet of a
	 * transmitted frame before a node must disable its EIA-485 driver: 15 bit
	 * times.
	 */
	 ...
	 * node must wait for a station to begin replying to a confirmed request:
	 * 255 milliseconds. (Implementations may use larger values for this
	 * timeout, not to exceed 300 milliseconds.)
	 */
	public static final int REPLY_TIMEOUT = 255;

	/**
	 * Repeater turnoff delay. The duration of a continuous logical one state at
	 * the active input port of an MS/TP repeater after which the repeater will
	 * enter the IDLE state: 29 bit times < Troff < 40 bit times.
	 */
	public static final int ROFF = 35;

	/**
	 * The width of the time slot within which a node may generate a token: 10
	 * milliseconds.
	 */
	...
	/**
	 * The maximum time a node may wait after reception of the token or a Poll
	 * For Master frame before sending the first octet of a frame: 15
	 * milliseconds.
	 */
	...
	/**
	 * The minimum time without a DataAvailable or ReceiveError event that a
	 * node must wait for a remote node to begin using a token or replying to a
	 * Poll For Master frame: 20 milliseconds. (Implementations may use larger
