...
abstract public class MstpNode implements Runnable {
	...
	/**
	 * The MAC address of this node. TS is generally read from a hardware DIP
	 * switch, or from nonvolatile memory. Valid values for TS are 0 to 254. The
	 * value 255 is used to denote broadcast when used as a destination address
	 * but is not allowed as a value for TS.
	 */
	...
	/**
	 * The flag ReceiveError is TRUE if an error is detected during the
	 * reception of an octet. Many common UARTs detect several types of receive
	 * errors, in particular framing errors and overrun errors. ReceiveError
	 * shall be TRUE if any of these errors is detected.
	 */
	...
	/**
	 * Used to count the number of received octets or errors. This is used in
	 * the detection of link activity.
	 */
	int eventCount;

	/**
	 * The number of frames sent by this node during a single token hold. When
	 * this counter reaches the value Nmax_info_frames, the node must pass the
	 * token.
	 */
	 ...
	 * InputBuffer is indexed from 0 to InputBufferSize-1. The maximum size of a
	 * frame is 501 octets. A smaller value for InputBufferSize may be used by
	 * some implementations.
	 */
	private final ByteQueue inputBuffer = new ByteQueue(MAX_FRAME_LENGTH);

	/**
	 * ReceivedInvalidFrame A Boolean flag set to TRUE by the Receive State
	 * Machine if an error is detected during the reception of a frame. Set to
	 * FALSE by the main state machine.
	 */
	...
	protected void sendFrame(Frame frame) {
		...
		try {
			...
			// Only write the same error message once. Prevents logs from
			// getting filled up unnecessarily with repeated
			// error messages.
		...
		// debug("Write took " + (timeSource.currentTimeMillis() - start) + "
		// ms");

		// NOTE: ??
		// Wait until the final stop bit of the most significant CRC octet has
		// been transmitted
