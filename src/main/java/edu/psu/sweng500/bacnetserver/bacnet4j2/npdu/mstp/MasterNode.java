...
public class MasterNode extends MstpNode {
	...
	/**
	 * The MAC address of the node to which This Station passes the token. If
	 * the Next Station is unknown, NS shall be equal to TS.
	 */
	private byte nextStation;

	/**
	 * The MAC address of the node to which This Station last sent a Poll For
	 * Master. This is used during token maintenance.
	 */
	...
	/**
	 * A Boolean flag set to TRUE by the master machine if this node is the only
	 * known master node.
	 */
	 ...
	 * additional master nodes. TokenCount is set to one at the end of the
	 * polling process.
	 */
	...
	public void setReplyFrame(FrameType type, byte destination, byte[] data) {
		synchronized (this) {
			if (state == MasterNodeState.answerDataRequest)
				...
				// The response has already exceeded the timeout, so just queue
				// it.
	...
	private void frame() {
		...
		if (type == null) {
			...
			// debug("Since last token possession: " + (now -
			// lastTokenPossession) + " ms");
			// else
	...
	private void useToken() {
		...
		if (frameToSend == null) {
			...
			if (frameToSend.getFrameType().oneOf(FrameType.testResponse, FrameType.bacnetDataNotExpectingReply)) {
				// SendNoWait
				// debug("useToken:SendNoWait to " +
				// frameToSend.frame.getDestinationAddress());
				if (LOG.isLoggable(Level.FINE))
	...
	private void waitForReply() {
		if (silence() > Constants.REPLY_TIMEOUT) {
			...
			// InvalidFrame
			// debug("waitForReply:InvalidFrame: '" + receivedInvalidFrame + "',
			// frame=" + frame);
			if (LOG.isLoggable(Level.FINE))
			...
			if (frame.forStation(thisStation)) {
				if (type.oneOf(FrameType.testResponse, FrameType.bacnetDataNotExpectingReply)) {
					// ReceivedReply
					// debug("waitForReply:ReceivedReply from " +
					...
					// ReceivedPostpone
					// debug("waitForReply:ReceivedPostpone from " +
					// frame.getSourceAddress());
					if (LOG.isLoggable(Level.FINE))
						LOG.fine(thisStation + " waitForReply:ReceivedPostpone");
					; // the reply to the message has been postponed until a
						// later time.
				}
	...
	/**
	 * The DONE_WITH_TOKEN state either sends another data frame, passes the
	 * token, or initiates a Poll For Master cycle.
	 */
	private void doneWithToken() {
		...
		if (frameCount < maxInfoFrames) {
			...
			// SendMaintenancePFM
			// debug("doneWithToken:SendMaintenancePFM to " +
			// adjacentStation(pollStation));
			if (LOG.isLoggable(Level.FINE))
		...
		// NOTE: variation from spec here ----vvv (i.e. added the "- 1")
			...
			// debug("doneWithToken:SoleMasterRestartMaintenancePFM: poll=" +
			// pollStation);
			if (LOG.isLoggable(Level.FINE))
	...
	/**
	 * The PASS_TOKEN state listens for a successor to begin using the token
	 * that this node has just attempted to pass.
	 */
	private void passToken() {
		...
		if (silence() < Constants.USAGE_TIMEOUT && eventCount > Constants.MIN_OCTETS) {
			...
			// FindNewSuccessor
			// debug("passToken:FindNewSuccessor: trying " +
			// adjacentStation(nextStation));
			if (LOG.isLoggable(Level.FINE))
	...
	/**
	 * The NO_TOKEN state is entered if SilenceTimer becomes greater than
	 * Tno_token, indicating that there has been no network activity for that
	 * period of time. The timeout is continued to determine whether or not this
	 * node may create a token.
	 */
	private void noToken() {
		...
		if (silence < delay && eventCount > Constants.MIN_OCTETS) {
			...
			state = MasterNodeState.idle;
			activity = true;
		} else if ((silence >= delay && silence < delay + Constants.SLOT) // Silence
																			// is
																			// in
																			// this
																			// master's
			...
			// GenerateToken
			// debug("noToken:GenerateToken: poll=" +
			// adjacentStation(thisStation));
			if (LOG.isLoggable(Level.FINE))
	...
	/**
	 * In the POLL_FOR_MASTER state, the node listens for a reply to a
	 * previously sent Poll For Master frame in order to find a successor node.
	 */
	private void pollForMaster() {
		if (receivedValidFrame) {
			if (frame.forStation(thisStation) && frame.getFrameType() == FrameType.replyToPollForMaster) {
				// ReceivedReplyToPFM
				// debug("pollForMaster:ReceivedReplyToPFM from " +
				// frame.getSourceAddress());
				if (LOG.isLoggable(Level.FINE))
			...
			if (nextStation != thisStation && longCondition) {
				// DoneWithPFM
				// debug("pollForMaster:DoneWithPFM passing token to " +
				// nextStation);
				if (LOG.isLoggable(Level.FINE))
				...
				if (adjacentStation(pollStation) != thisStation && longCondition) {
					// SendNextPFM
					// debug("pollForMaster:SendNextPFM to " +
					// adjacentStation(pollStation));
					if (LOG.isLoggable(Level.FINE))
	...
	/**
	 * The ANSWER_DATA_REQUEST state is entered when a BACnet Data Expecting
	 * Reply, a Test_Request, or a proprietary frame that expects a reply is
	 * received.
	 */
