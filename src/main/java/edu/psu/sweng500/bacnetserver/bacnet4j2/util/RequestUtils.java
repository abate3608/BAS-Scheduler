...
import edu.psu.sweng500.bacnetserver.bacnet4j2.service.acknowledgement.ReadPropertyMultipleAck;
import edu.psu.sweng500.bacnetserver.bacnet4j2.service.confirmed.ReadPropertyMultipleRequest;
import edu.psu.sweng500.bacnetserver.bacnet4j2.service.confirmed.ReadPropertyRequest;
import edu.psu.sweng500.bacnetserver.bacnet4j2.service.confirmed.WritePropertyRequest;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.type.constructed.ReadAccessResult.Result;
...
import edu.psu.sweng500.bacnetserver.bacnet4j2.type.enumerated.AbortReason;
...
public class RequestUtils {
	...
	public static Encodable sendReadPropertyAllowNull(LocalDevice localDevice, RemoteDevice d, ObjectIdentifier oid,
		...
		try {
			ReadPropertyAck ack = (ReadPropertyAck) localDevice.send(d,
					new ReadPropertyRequest(oid, pid, propertyArrayIndex));
			if (callback != null)
	...
	public static void getExtendedDeviceInformation(LocalDevice localDevice, RemoteDevice d) throws BACnetException {
		...
		// d.setProtocolRevision((UnsignedInteger) values.getNullOnError(oid,
	 ...
	 * @throws BACnetException
	 */
	public static List<Pair<ObjectPropertyReference, Encodable>> readProperties(LocalDevice localDevice, RemoteDevice d,
			List<ObjectPropertyReference> oprs, RequestListener callback) throws BACnetException {
		PropertyReferences refs = new PropertyReferences();
				...
				try {
					...
					if (e.getApdu().getAbortReason() == AbortReason.bufferOverflow.intValue()
						...
						if (counter > 0)
							...
							// Failed on the first partition. Send all one at a
							// time, reduce the device's max
							// references, and quit.
		...
		for (ObjectIdentifier oid : properties.keySet()) {
			...
			for (PropertyReference ref : refList) {
				...
				try {
					...
					updater.increment(oid, ref.getPropertyIdentifier(), ref.getPropertyArrayIndex(),
							new BACnetError(ErrorClass.communication, ErrorCode.timeout));
				} catch (ErrorAPDUException e) {
					updater.increment(oid, ref.getPropertyIdentifier(), ref.getPropertyArrayIndex(),
							e.getBACnetError());
				}
